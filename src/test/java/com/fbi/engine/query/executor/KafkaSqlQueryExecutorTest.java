...
public class KafkaSqlQueryExecutorTest {
    ...
    public void executeSelectQuery() throws Exception {
        ...
        when(restTemplate.exchange(eq("http://localhost:1234/query"), eq(HttpMethod.POST),
            any(HttpEntity.class), any(Class.class))).thenReturn(ResponseEntity.ok("{\"row\":{\"columns\":[1524760769983,\"1\",1524760769747,\"alice\",\"home\"]},\"errorMessage\":null}" +
            "\n" +
    ...
    public void executeSelectQueryWithMetadata() throws Exception {
        ...
        when(restTemplate.exchange(eq("http://localhost:1234/ksql"), eq(HttpMethod.POST),
            any(HttpEntity.class), any(Class.class))).thenReturn(ResponseEntity.ok(
            new KafkaKsqlDescribeResponse[]{new KafkaKsqlDescribeResponse()
                .setSourceDescription(new KafkaKsqlDescribeResponse.SourceDescription()
                .setFields(Arrays.asList(new KafkaKsqlDescribeResponse.Field()
                    .setName("first")
                    .setSchema(new KafkaKsqlDescribeResponse.Schema()
                        .setType("firstType")),
                    new KafkaKsqlDescribeResponse.Field()
                        .setName("second")
                        .setSchema(new KafkaKsqlDescribeResponse.Schema()
                            .setType("secondType")),
                    new KafkaKsqlDescribeResponse.Field()
    ...
    public void executeSelectShowTablesQuery() throws Exception {
        ...
        when(restTemplate.exchange(eq("http://localhost:1234/ksql"), eq(HttpMethod.POST),
            any(HttpEntity.class), any(Class.class))).thenReturn(ResponseEntity.ok(
            new KafkaShowTablesResponse[]{new KafkaShowTablesResponse()
                .setTables(Arrays.asList(new KafkaShowTablesResponse.Table()
                   .setName("table1"),
                new KafkaShowTablesResponse.Table()
                    .setName("table2")))
                .setStreams(Arrays.asList(new KafkaShowTablesResponse.Table()
                    .setName("table1"),
                new KafkaShowTablesResponse.Table()
                    .setName("stream2")))
            }
