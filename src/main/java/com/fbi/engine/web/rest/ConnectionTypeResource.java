...
public class ConnectionTypeResource {
     ...
     * @return the ResponseEntity with status 201 (Created) and with body the new connectionTypeDTO, or with status 400 (Bad Request) if the connectionType has already an ID
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PostMapping("/connection-types")
    @Timed
    public ResponseEntity<ConnectionTypeDTO> createConnectionType(@Valid @RequestBody ConnectionTypeDTO connectionTypeDTO) throws URISyntaxException {
        log.debug("REST request to save ConnectionType : {}", connectionTypeDTO);
        if (connectionTypeDTO.getId() != null) {
            return ResponseEntity.badRequest().headers(HeaderUtil.createFailureAlert(ENTITY_NAME, "idexists", "A new connectionType cannot already have an ID")).body(null);
        }
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the updated connectionTypeDTO,
     * or with status 400 (Bad Request) if the connectionTypeDTO is not valid,
     * or with status 500 (Internal Server Error) if the connectionTypeDTO couldn't be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PutMapping("/connection-types")
    @Timed
    public ResponseEntity<ConnectionTypeDTO> updateConnectionType(@Valid @RequestBody ConnectionTypeDTO connectionTypeDTO) throws URISyntaxException {
     ...
     * @return the ResponseEntity with status 200 (OK) and the list of connectionTypes in body
     */
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the connectionTypeDTO, or with status 404 (Not Found)
     */
