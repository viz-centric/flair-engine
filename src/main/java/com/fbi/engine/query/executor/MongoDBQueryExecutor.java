...
public class MongoDBQueryExecutor extends SqlQueryExecutor {
    ...
    public void execute(Query query, Writer writer) {
        ...
        MongoClient mongo = new MongoClient(connection.getDetails().getServerIp(), connection.getDetails().getServerPort());

        DB db = mongo.getDB(connection.getDetails().getDatabaseName());
