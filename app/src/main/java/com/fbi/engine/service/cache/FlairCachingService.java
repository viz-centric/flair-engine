...
public class FlairCachingService {
	...
	public Optional<CacheMetadata> getResult(FlairQuery query, String connectionLinkId) {
		log.info("Making a grpc caching request for connection {} query {}", connectionLinkId, query);
		...
		try {
			final GetCacheResponse cache = getCacheServiceStub().getCache(com.flair.bi.messages.GetCacheRequest
					.newBuilder().setKey(cacheKey.get()).setTable(connectionLinkId).build());
			...
			log.error("Error fetching data from cache " + e.getMessage() + " status " + e.getStatus() + " trailers "
					+ e.getTrailers(), e);
			return Optional.empty();
		} catch (Exception e) {
	...
	public void putResult(FlairQuery query, String connectionLinkId, String result, CacheParams cacheParams) {
		...
		try {
			getCacheServiceStub()
					.putCache(com.flair.bi.messages.PutCacheRequest.newBuilder().setKey(cacheKey.get())
							.setTable(connectionLinkId).setValue(result)
							.setRefreshAfterDate(Instant.now()
									.plus(cacheParams.getRefreshAfterMinutes(), ChronoUnit.MINUTES).getEpochSecond())
							.setRefreshAfterCount(cacheParams.getRefreshAfterTimesRead())
							.setPurgeAfterDate(Instant.now()
									.plus(cacheParams.getCachePurgeAfterMinutes(), ChronoUnit.MINUTES).getEpochSecond())
							.build());
		} catch (StatusRuntimeException e) {
			log.error("Error saving data to cache " + e.getMessage() + " status " + e.getStatus() + " trailers "
					+ e.getTrailers(), e);
		} catch (Exception e) {
