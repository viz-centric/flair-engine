...
public final class QueryGrpcUtils {
	...
	private static List<FieldDTO> toFieldDTOs(List<Query.Field> fieldsList) {
		return fieldsList.stream().map(QueryGrpcUtils::toFieldDTO).collect(Collectors.toList());
	}
	...
	private static List<HavingDTO> getListHavingDTO(List<Query.HavingHolder> havingList) {
		return havingList.stream()
				.map(h -> HavingDTO.builder().feature(toFieldDTO(h.getFeature())).value(h.getValue())
						.comparatorType(HavingDTO.ComparatorType.valueOf(h.getComparatorType().name())).build())
	...
	private static List<ConditionExpressionDTO> getListConditionExpressionDTO(
			List<Query.ConditionExpressionHolder> conditionExpressions) {
		return conditionExpressions.stream().map(conditionExpressionHolder -> {
			...
			conditionExpressionDTO.setConditionExpression((conditionExpressionHolder.getExpressionTypeValue() > 1)
					? createConditionExpression(conditionExpressionHolder.getConditionExpression(),
							conditionExpressionHolder.getExpressionTypeValue())
					: createAndOrConditionExpression(conditionExpressionHolder.getConditionExpression(),
							conditionExpressionHolder.getExpressionTypeValue(),
							conditionExpressionHolder.getAndOrExpressionType()));
	...
	private static ConditionExpression createConditionExpression(String conditionExpressionString,
			Integer expressionType) {
		return JacksonUtil.fromString(conditionExpressionString,
	...
	private static ConditionExpression createAndOrConditionExpression(String conditionExpressionString,
		...
		if (jsonTree.isJsonObject()) {
			...
			return (expressionType == 0) ? createAndConditionExpression(firstExpression, secondExpression)
					: createOrConditionExpression(firstExpression, secondExpression);
		}
		return null;
	}

	private static ConditionExpression createAndConditionExpression(ConditionExpression firstConditionExpression,
			ConditionExpression secondConditionExpression) {
	...
	private static ConditionExpression createOrConditionExpression(ConditionExpression firstConditionExpression,
			ConditionExpression secondConditionExpression) {
