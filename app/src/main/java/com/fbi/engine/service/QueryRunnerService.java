...
public class QueryRunnerService {
	...
	public RunQueryResultDTO runQuery(QueryDTO queryDTO, String datasourceId) {
		...
		if (conn == null) {
			return new RunQueryResultDTO().setResultCode(RunQueryResultDTO.Result.DATASOURCE_NOT_FOUND);
		}

		FlairQuery query = new FlairQuery(queryDTO.interpret(), queryDTO.isMetaRetrieved(), queryDTO.getSource());
		...
		if (StringUtils.isNotEmpty(executeQuery)) {
			return new RunQueryResultDTO().setResultCode(RunQueryResultDTO.Result.OK).setRawResult(executeQuery);
		}

		return new RunQueryResultDTO().setResultCode(RunQueryResultDTO.Result.INVAILD_QUERY);
	}
	...
	public CompileQueryResultDTO compileQuery(QueryDTO queryDTO, String datasourceId) {
		...
		return new CompileQueryResultDTO().setRawQuery(queryResult.getQuery());
	}
}
