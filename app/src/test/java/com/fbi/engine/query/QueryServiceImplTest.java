...
public class QueryServiceImplTest {
	...
	public void executeQueryWithReadCache() throws Exception {
		...
		CacheMetadata cacheMetadata = service.executeQuery(connection, flairQuery,
				new CacheParams().setReadFromCache(true));
	...
	public void executeQueryWithReadCacheIsDisabled() throws Exception {
		...
		CacheMetadata cacheMetadata = service.executeQuery(connection, flairQuery,
				new CacheParams().setReadFromCache(false).setWriteToCache(true));

		assertEquals("some result", cacheMetadata.getResult());
		verify(flairCachingService, times(1)).putResultAsync(eq(flairQuery), eq(connection.getLinkId()),
				eq("some result"), any(CacheParams.class));
	}
	...
	public void executeQueryWithReadCacheCausesToReadDbIfNoCache() throws Exception {
		...
		CacheMetadata cacheMetadata = service.executeQuery(connection, flairQuery,
				new CacheParams().setReadFromCache(true).setWriteToCache(true));
