...
public abstract class SqlQueryExecutor implements QueryExecutor {
  ...
  private static final Map<ConnectionDataKey, ConnectionDataValue> connections =
  ...
  protected Driver initDriver() throws ExecutionException {
    try {
      final Driver d =
          strategy.loadDriver(getDriverClassName(), driver == null ? getDefaultDriver() : driver);
      log.info("Driver successfully registered: {}", getDriverClassName());
      return d;
    } catch (DriverLoadingException e) {
      log.error(
          "An error occured trying to load driver for SQL Query executor for driver class name: {}",
          getDriverClassName());
  ...
  public void execute(Query query, Writer writer) throws ExecutionException {
    ...
    final String connectionUsername =
        this.connection.getConnectionProperties().getProperty(USERNAME);
    final String connectionPassword =
        this.connection.getConnectionProperties().getProperty(PASSWORD);

    final ConnectionDataValue connectionDataValue =
        getConnection(
            connectionString,
            connectionUsername,
            connectionPassword,
      ...
      if (c != null) {
        try (final Statement statement = c.createStatement()) {
          ...
          try (ResultSet resultSet = statement.getResultSet()) {
            writer.write(
                new ResultSetConverter(objectMapper, query.isMetadataRetrieved())
                    .convert(resultSet));
          }
        }
        ...
        log.error(
            "Failed to make connection! Connection string: {}", connection.getConnectionString());
  ...
  private static ConnectionDataValue getConnection(
  ...
  private static ConnectionDataValue createConnectionValue(
      ConnectionDataKey connectionData, Properties properties) {
    return new ConnectionDataValue(createHikariConnection(connectionData, properties));
  }

  private static DataSource createHikariConnection(
      ConnectionDataKey connectionData, Properties properties) {
